...
const CourseHeader = ({
  ...
  <Link href={`/account/edupro/${slug}`}>
    <div className="mx-auto max-w-7xl relative border-r-4 border-sky-600 px-4 py-2 sm:py-4 rounded-lg shadow-sm mb-4 min-h-[100px] sm:min-h-[120px] flex items-center group hover:bg-blue-50 cursor-pointer transition-colors duration-200 bg-white">
      <ArrowUpIcon
      ...
      <div className="hidden sm:flex sm:flex-col space-y-0 flex-1 items-center justify-center">
        ...
        <span className="sm:text-sm text-sm font-light text-gray-500 ">
          Topics
        </span>
      </div>
    </div>
...
const TopicHeader = ({
  ...
  >
    <div className="flex flex-col space-y-0 flex-1">
      ...
      <div className="my-4">
        <h3 className="text-lg sm:text-xl font-bold text-sky-600 text-center px-4">
          {topic.title}
        </h3>
        {topic.description && (
          <p className="hidden sm:block text-sm text-gray-600 text-center">
            {topic.description}
          </p>
        )}
      </div>
      ...
      <ul className="mt-2 grid grid-cols-1 lg:grid-cols-3 gap-8 sm:gap-x-16 px-8">
        {lessons.map((lesson) => (
          ...
          >
            ...
            <Link
              href={`/account/edupro/${slug}/topic/${topicSlug}/lesson/${lesson.id}`}
            >
...
export default function EduProTopicDetail() {
  ...
  const { slug, topicSlug } = useParams() as {
    slug: string;
    topicSlug: string;
  };
  ...
  const handleTabChange: Dispatch<SetStateAction<Tab>> = (tabValue) => {
    const newTab =
      typeof tabValue === "string" ? tabValue : tabValue(activeTab);
  ...
  const fetchLessonProgress = async (lessonIds: number[]) => {
    ...
    try {
      ...
      if (error)
        throw new Error(`Error fetching lesson progress: ${error.message}`);
      ...
      const allCompleted =
        lessonIds.length > 0 && lessonIds.every((id) => progressMap[id]);
      ...
      if (newCompletedStatus) {
        ...
        for (const lessonId of lessonIds) {
          ...
          if (fetchError && fetchError.code !== "PGRST116") {
            throw new Error(
              `Error checking existing lesson progress: ${fetchError.message}`
            );
          }
          ...
          if (existingProgress) {
            ...
            if (updateError) {
              throw new Error(
                `Error updating lesson completion: ${updateError.message}`
              );
            }
          } else {
            ...
            if (insertError) {
              throw new Error(
                `Error inserting lesson completion: ${insertError.message}`
              );
            }
          }
        }
  ...
  const fetchTopicDetails = async () => {
    ...
    try {
      ...
      if (!isStudent)
        throw new Error("Access denied: You are not enrolled as a student.");
      ...
      if (courseError || !courseData) {
        ...
        if (associatedCourse) {
          ...
          router.push(
            `/account/edupro/${associatedCourse.slug}/topic/${topicSlug}`
          );
          return;
        }
      ...
      const { data: allTopicsData, error: allTopicsError } = (await supabase
        ...
        data: CourseTopicResponse[] | null;
        error: any;
      };

      if (allTopicsError)
        throw new Error(`Error fetching topics: ${allTopicsError.message}`);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-4 mb-4 grid grid-cols-3">
            ...
            {topic && (
              <div
                ...
                className={`flex items-center justify-center w-6 h-6 border-2 ${
                  ...
                  session
                    ? "cursor-pointer hover:scale-110 hover:bg-teal-50"
                    : "cursor-default"
                } group-hover:ring-2 group-hover:ring-teal-300 group`}
                aria-label={
                  isTopicCompleted ? "Mark as Incomplete" : "Mark as Complete"
                }
                title={
                  isTopicCompleted ? "Mark as Incomplete" : "Mark as Complete"
                }
              >
                ...
                {session && (
                  <CheckIcon className="hidden group-hover:block w-4 h-4" />
                )}
              </div>
            )}
          </div>
          <div className="flex justify-center sm:justify-end items-center space-x-2">
            <button
              onClick={() =>
                previousTopic && navigateToTopic(previousTopic.topic.slug)
              }
