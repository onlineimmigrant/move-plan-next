...
async function fetchHomePageData(baseUrl: string): Promise<HomePageData> {
  ...
  try {
    ...
    const [heroResponse, brandsResponse, faqsResponse, templateSectionsResponse, templateHeadingSectionsResponse] = await Promise.all([
      fetch(`${baseUrl}/api/hero?organizationId=${organizationId}`, { cache: 'force-cache' }),
      fetch(`${baseUrl}/api/brands?organizationId=${organizationId}`, { cache: 'force-cache' }),
      fetch(`${baseUrl}/api/faqs?organizationId=${organizationId}`, { cache: 'force-cache' }),
      fetch(`${baseUrl}/api/template-sections?url_page=/home&organizationId=${organizationId}`, { cache: 'force-cache' }),
      fetch(`${baseUrl}/api/template-heading-sections?url_page=/home&organizationId=${organizationId}`, { cache: 'force-cache' }),
    ]);
