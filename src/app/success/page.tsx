...
export default function SuccessPage() {
  ...
  const paymentIntentId = searchParams.get('payment_intent'); // Use payment_intent instead of session_id
        ...
        .then(async (data) => {
          ...
          if (data.status === 'succeeded') {
            try {
              ...
              setError('User creation skipped due to database issue: ' + (err.message || 'Unknown error'));
            }
          }
        })
        .catch((err) => {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 text-center">
      ...
      <div id="payment-status">
        ...
        {error && (
          <div className="mb-6 text-red-500 text-sm font-medium">{error}</div>
        )}
        {userCreated && (
        ...
        {paymentIntent && (
          <a
            ...
            className="text-sky-600 hover:text-sky-700 text-sm font-medium inline-block transition-colors duration-200"
          >
            View details
            <svg
              width="15"
        ...
        <Link href="/" id="retry-button" className="text-sky-600 hover:text-sky-700 text-sm font-medium inline-block transition-colors duration-200 mt-4">
          Test another
        </Link>
      </div>
    </div>
