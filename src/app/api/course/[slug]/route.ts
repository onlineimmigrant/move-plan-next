...
export async function GET(request: NextRequest, context: { params: { slug: string } }) {
  ...
  console.log('Received GET request for /api/course/[slug]:', slug, 'organization_id:', organizationId);
  ...
  try {
    ...
    if (postData.section_id !== 'Course') {
      console.log('Post does not belong to section_id Course:', slug);
      return NextResponse.json(
        { error: 'This route is only for posts with section_id Course' },
        { status: 403 }
      );
    }
    ...
    console.error('Error in GET /api/course/[slug]:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  ...
  console.log('Received POST request for /api/course/[slug]');
  ...
  try {
    ...
    const {
      ...
      main_photo = null,
      section_id = 'Course', // Default to Course for this route
      subsection = null,
    ...
    if (section_id !== 'Course') {
      ...
      return NextResponse.json(
        { error: 'This route requires section_id to be Course' },
        { status: 400 }
      );
    }
    ...
    console.error('Error in POST /api/course/[slug]:', error);
    return NextResponse.json(
      { error: 'Internal server error', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
...
export async function PATCH(request: NextRequest, context: { params: { slug: string } }) {
  ...
  const { slug } = context.params;
  console.log('Received PATCH request for /api/course/[slug]:', slug);
  ...
  try {
    ...
    if (existingPost.section_id !== 'Course') {
      console.log('Post does not belong to section_id Course:', slug);
      return NextResponse.json(
        { error: 'This route is only for posts with section_id Course' },
        { status: 403 }
    ...
    if (section_id !== undefined && section_id !== 'Course') {
      console.log('Attempt to change section_id to non-Course value:', section_id);
      return NextResponse.json(
        { error: 'This route requires section_id to remain Course' },
        { status: 400 }
      );
    }
    ...
    console.error('Error in PATCH /api/course/[slug]:', error);
    return NextResponse.json(
      { error: 'Internal server error', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
