...
export async function GET(request: NextRequest, context: { params: Promise<{ slug: string }> }) {
  ...
  console.log('Received GET request for /api/sqe-2/practice-area/[slug]:', slug, 'organization_id:', organizationId);
  ...
  try {
    ...
    if (postData.section_id !== 'Practice Area') {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const {
      ...
      main_photo = null,
      section_id = 'Practice Area', // Default to Practice Area
      subsection = null,
...
export async function PATCH(request: NextRequest, context: { params: { slug: string } }) {
  ...
  try {
    ...
    if (existingPost.section_id !== 'Practice Area') {
      console.log('Post does not belong to section_id Practice Area:', slug);
      return NextResponse.json(
        { error: 'This route is only for posts with section_id Practice Area' },
        { status: 403 }
    ...
    if (section_id !== undefined && section_id !== 'Practice Area') {
