...
export async function GET(request: NextRequest, context: { params: { slug: string } }) {
  ...
  console.log('Received GET request for /api/sqe-2/[slug]:', slug, 'organization_id:', organizationId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const {
      ...
      main_photo = null,
      section_id = 'SQE2', // Default to SQE2 for this route
      subsection = null,
...
export async function PATCH(request: NextRequest, context: { params: { slug: string } }) {
  ...
  try {
    ...
    if (existingPost.section_id !== 'SQE2') {
      console.log('Post does not belong to section_id SQE2:', slug);
      return NextResponse.json(
        { error: 'This route is only for posts with section_id SQE2' },
        { status: 403 }
    ...
    if (section_id !== undefined && section_id !== 'SQE2') {
