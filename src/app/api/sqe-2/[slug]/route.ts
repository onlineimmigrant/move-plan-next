...
export async function GET(_request: NextRequest, context: any) {
  ...
  console.log('Received GET request for /api/sqe-2/[slug]:', slug);
  ...
  try {
    ...
    if (postData.section_id !== 'SQE2') { // Changed to 'SQE2'
      console.log('Post does not belong to section_id SQE2:', slug);
      return NextResponse.json(
        { error: 'This route is only for posts with section_id SQE2' },
        { status: 403 }
      );
    }
    ...
    console.error('Error in GET /api/sqe-2/[slug]:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  ...
  console.log('Received POST request for /api/sqe-2/[slug]');
  ...
  try {
    ...
    if (section_id !== 'SQE2') { // Changed to 'SQE2'
      console.log('Invalid section_id for this route:', section_id);
      return NextResponse.json(
        { error: 'This route requires section_id to be SQE2' },
        { status: 400 }
      );
    }
    ...
    console.error('Error in POST /api/sqe-2/[slug]:', error);
    return NextResponse.json(
      { error: 'Internal server error', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
...
export async function PATCH(request: NextRequest, context: any) {
  ...
  const { slug } = context.params;
  console.log('Received PATCH request for /api/sqe-2/[slug]:', slug);
  ...
  try {
    ...
    if (existingPost.section_id !== 'SQE2') { // Changed to 'SQE2'
      console.log('Post does not belong to section_id SQE2:', slug);
      return NextResponse.json(
        { error: 'This route is only for posts with section_id SQE2' },
        { status: 403 }
    ...
    if (section_id !== undefined && section_id !== 'SQE2') { // Changed to 'SQE2'
      console.log('Attempt to change section_id to non-SQE2 value:', section_id);
      return NextResponse.json(
        { error: 'This route requires section_id to remain SQE2' },
        { status: 400 }
      );
    }
    ...
    console.error('Error in PATCH /api/sqe-2/[slug]:', error);
    return NextResponse.json(
      { error: 'Internal server error', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
