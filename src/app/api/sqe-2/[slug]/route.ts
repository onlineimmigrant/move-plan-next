// app/api/sqe-2/topic/[slug]/route.ts
...
export async function GET(request: Request, { params }: { params: { slug: string } }) {
  ...
  try {
    ...
    if (postData.section_id !== 3) {
      console.log('Post does not belong to section_id 3:', slug);
      return NextResponse.json(
        { error: 'This route is only for posts with section_id 3' },
        { status: 403 }
      );
    }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (section_id !== 3) {
      ...
      return NextResponse.json(
        { error: 'This route requires section_id to be 3' },
        { status: 400 }
      );
    }
...
export async function PATCH(request: Request, { params }: { params: { slug: string } }) {
  ...
  try {
    ...
    if (section_id !== undefined && section_id !== 3) {
      console.log('Attempt to change section_id to non-3 value:', section_id);
      return NextResponse.json(
        { error: 'This route requires section_id to remain 3' },
        { status: 400 }
      );
    }
