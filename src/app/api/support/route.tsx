...
export async function GET(request: NextRequest) {
  ...
  console.log('Received GET request for /api/support');
  ...
  try {
    ...
    console.log('Fetching support content from Supabase for organization_id:', organizationId);
    const { data: supportData, error: supportError } = await supabase
      ...
      .eq('subsection', 'Support')
      .or(`organization_id.eq.${organizationId},organization_id.is.null`)
      .order('created_on', { ascending: false });

    if (supportError) {
      console.error('Supabase query error:', supportError);
      return NextResponse.json(
        { error: 'Failed to fetch support content', details: supportError.message },
        { status: 500 },
      );
    }

    supportData.forEach(post => console.log(`Support ID ${post.id} subsection:`, post.subsection, 'organization_id:', post.organization_id));
    console.log('Fetched support content:', supportData);
...
export async function POST(request: NextRequest) {
  ...
  console.log('Received POST request for /api/support');
  ...
  try {
    ...
    console.log('Inserting new support content into Supabase...');
    ...
    if (insertError) {
      ...
      return NextResponse.json(
        { error: 'Failed to create support content', details: insertError.message },
        { status: 500 },
      );
    }

    console.log('New support content created:', newSupport);
