...
type DisclosureKey = "tables" | "custom";

interface ReportsChildMenuProps {
...
const sectionIcons: Record<DisclosureKey, React.ComponentType<React.SVGProps<SVGSVGElement>>> = {
  tables: DocumentChartBarIcon, // Icon for table-specific reports
  custom: DocumentChartBarIcon, // Icon for custom report constructor
};
  ...
  return link.children?.length ? (
    ...
    <Link href={link.href} onClick={onClick} className={linkClasses}>
      <span className={cn("text-xs w-4 h-4 flex justify-center", iconClasses)}>
        <ChartBarIcon /> {/* Icon for individual report links */}
      </span>
...
export default function ReportsChildMenu({
  isSidebarOpen,
  setIsSidebarOpen,
  sidebarLinks,
  openSections,
