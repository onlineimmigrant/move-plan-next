  ...
  useEffect(() => {
    const fetchSupport = async () => {
      try {
        ...
        const response = await fetch(`/api/support?organization_id=${organizationId}`);
        if (response.ok) {
          ...
          console.error('Failed to fetch support content:', response.status, response.statusText, errorData);
          setError(errorData.error || 'Failed to fetch support content');
        }
      } catch (error) {
  ...
  return (
    <div className="bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-16 sm:py-24">
        ...
        {posts.length === 0 ? (
          <div className="text-center py-16 text-gray-500">No support content available</div>
        ) : filteredPosts.length === 0 && searchQuery ? (
          <div className="text-center py-16 text-gray-500">No support content found matching "{searchQuery}"</div>
        ) : (
