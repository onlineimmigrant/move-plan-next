...
export const Map: React.FC = () => {
  ...
  // Load Yandex Maps API script once using centralized loader
  useEffect(() => {
    loadYandexMapsAPI()
      .then(() => {
  ...
  return (
    <div className="w-full h-full min-h-[400px] relative">
      ...
      <div className="mt-4 bg-white rounded-lg p-4 shadow-sm border border-gray-200">
        <h4 className="font-semibold text-gray-800 mb-3">Транспортная доступность</h4>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
          <div className="flex items-center space-x-2">
            <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
            <span className="text-gray-600">Метро:</span>
            <span className="text-gray-800 font-medium">5 минут пешком</span>
          </div>
          <div className="flex items-center space-x-2">
            <div className="w-2 h-2 bg-green-500 rounded-full"></div>
            <span className="text-gray-600">Автобус:</span>
            <span className="text-gray-800 font-medium">2 минуты пешком</span>
          </div>
          <div className="flex items-center space-x-2">
            <div className="w-2 h-2 bg-yellow-500 rounded-full"></div>
            <span className="text-gray-600">Парковка:</span>
            <span className="text-gray-800 font-medium">Подземная</span>
          </div>
          <div className="flex items-center space-x-2">
            <div className="w-2 h-2 bg-purple-500 rounded-full"></div>
            <span className="text-gray-600">Аэропорт:</span>
            <span className="text-gray-800 font-medium">45 минут</span>
          </div>
        </div>
      </div>
    </div>
