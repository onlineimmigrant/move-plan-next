/**
 * HeaderEditModal - Modern header editing modal
 * Features: 5 header types, logo upload, mega menu, live preview
 */
...
function HeaderEditModal() {
  ...
  const [imageSelectCallback, setImageSelectCallback] = useState<((url: string) => void) | null>(null);
  const [previewMode, setPreviewMode] = useState<'desktop' | 'mobile'>('desktop');
  
  const [saveError, setSaveError] = useState<string | null>(null);
  const [selectedStyle, setSelectedStyle] = useState(headerStyleFull?.type || headerStyle || 'default');
  ...
  useEffect(() => {
    const actualStyle = headerStyleFull?.type || headerStyle || 'default';
  ...
  const handleImageGalleryOpen = (callback: (imageUrl: string) => void) => {
  ...
  return (
    <>
      ...
      >
        ...
        {/* Menu Button Panel */}
        <div className="px-4 md:px-6 py-3 flex items-center justify-between border-b border-white/10 dark:border-gray-700/20 bg-white/30 dark:bg-gray-800/30 relative z-30">
            ...
            {[
              ...
              <div key={menu.id} className="relative">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-3 md:px-4 py-2 rounded-full font-medium text-xs md:text-sm transition-all duration-300 shadow-sm whitespace-nowrap"
                  style={
                    openMenu === menu.id
                      ? {
          ...
          <div className="inline-flex items-center gap-1 bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
            <button
              ...
              style={previewMode === 'desktop' ? { color: primary.base } : {}}
            ...
            <button
              ...
              style={previewMode === 'mobile' ? { color: primary.base } : {}}
            ...
            <>
              ...
              {openMenu && (
                <>
                  ...
                  <div 
                    className="absolute inset-x-0 bg-white dark:bg-gray-800 shadow-2xl z-50 rounded-b-2xl"
                    style={{ 
                      top: '140px', 
                    ...
                    <div className={`w-full py-4 md:py-6 relative ${openMenu === 'translations' ? 'px-4 md:px-6' : 'px-4 md:px-6'}`}>
                        ...
                        {openMenu === 'style' && (
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                            ...
                            <StyleSection
                              ...
                              headerStyleFull={headerStyleFull}
                              organizationId={organizationId || ''}
                              onStyleChange={setSelectedStyle}
                              onStyleFullChange={updateHeaderStyleFull}
                            />
                      ...
                      <div className="-mx-4 md:-mx-6 -mb-4 md:-mb-6">
                        <div className="px-4 md:px-6 mb-3">
                          <h3 className="text-xs md:text-sm font-semibold text-gray-700 dark:text-gray-300">Live Preview</h3>
                        </div>
              ...
              {/* Full Width Header Preview (when no menu is open) */}
              {!openMenu && (
