 ...
 * Enhanced for HeaderEditModal with image and description fields
 */
...
// Sortable Submenu Row Component - Enhanced with Image and Description
function SortableSubmenuRow({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="w-full flex items-start gap-2 px-2 py-2 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 rounded transition-colors"
    >
      ...
      {/* Image Thumbnail */}
      <button
        onClick={() => onImageClick(menuItemId, submenu.id, submenu.image || undefined)}
        className="flex-shrink-0 w-10 h-10 rounded border border-gray-200 dark:border-gray-700 overflow-hidden hover:border-gray-400 dark:hover:border-gray-500 transition-colors bg-gray-100 dark:bg-gray-800 flex items-center justify-center"
      ...
      <div className="flex-1 min-w-0">
        ...
        <div 
          className="font-medium cursor-pointer hover:text-opacity-80 truncate"
          onDoubleClick={(e) => {
            e.stopPropagation();
            onInlineEditOpen(menuItemId, 'submenu_name', submenu.name, submenu.id);
          }}
        ...
        <div 
          className="text-[11px] text-gray-500 dark:text-gray-400 cursor-pointer hover:text-opacity-80 line-clamp-2 mt-0.5"
          onDoubleClick={(e) => {
            e.stopPropagation();
            onInlineEditOpen(menuItemId, 'submenu_description', submenu.description || '', submenu.id);
          }}
          title="Double-click to edit description"
        >
          {submenu.description || 'No description'}
        </div>
      </div>
      ...
      <button
        onClick={(e) => {
          e.stopPropagation();
          onInlineEditOpen(menuItemId, 'submenu_url', submenu.url_name, submenu.id);
        }}
        className="flex-shrink-0 p-1 text-gray-400 dark:text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 rounded transition-all mt-1"
        style={{
          color: primary.base
        }}
        onMouseEnter={(e) => {
      ...
      <button
        ...
        className="flex-shrink-0 p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded transition-all mt-1"
      >
      ...
      <button
        ...
        className="flex-shrink-0 p-1 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-all mt-1"
      >
    ...
    >
      ...
      <div className="p-3 space-y-2 flex-1">
        ...
        <div className="pt-2 border-t border-gray-100 dark:border-gray-700/50 space-y-1">
          {/* Display Submenu Items with drag handle, image, name, description, and URL icon */}
          {item.submenu_items && item.submenu_items.length > 0 && (
            <DndContext
                ...
                <div className="space-y-1">
                  {item.submenu_items.map((submenu: SubMenuItem) => (
                    <SortableSubmenuRow
                      ...
                      onImageClick={onSubmenuImageClick}
                    />
                  ))}
                </div>
              </SortableContext>
