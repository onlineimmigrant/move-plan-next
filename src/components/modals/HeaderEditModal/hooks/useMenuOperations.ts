...
export function useMenuOperations({
  ...
  /**
   * Handles submenu field updates including image and description
   * Image field accepts image URL from ImageGalleryModal
   */
  const handleSubmenuEdit = async (
    ...
    try {
      ...
      if (field === 'image') {
        toast.success('Submenu image updated successfully');
      }
    } catch (error) {
  ...
  /**
   * Creates a new submenu item with optional URL, description, and image
   * Auto-generates URL slug from name if not provided
   */
